...
const IDENTITY_FILE = 'C:/Users/doris/.openclaw/identity/device.json';
...
ws.on('message', (data) => {
  ...
  // Skip noisy events
  if (msg.type === 'event' && (msg.event === 'session.tool' || msg.event === 'tick' || msg.event === 'health')) return;
  ...
  if (msg.type === 'res' && msg.payload?.type === 'hello-ok' && !authed) {
    ...
    setTimeout(() => {
      ...
      ws.send(JSON.stringify({ type: 'req', id: 'chat-1', method: 'chat.send', params: {
        sessionKey: 'agent:main:main', message: 'say hello in 3 words',
  ...
  if (msg.type === 'res' && msg.id === 'sub-1') {
    console.log('[SUB RES]', JSON.stringify(msg));
    return;
  }
  
  if (msg.type === 'res' && msg.id === 'chat-1') {
    console.log('[CHAT RES]', JSON.stringify(msg.payload).substring(0, 200));
    return;
  }
  
  if (msg.type === 'event' && msg.event === 'chat') {
    console.log('[CHAT EVENT]', JSON.stringify(msg.payload).substring(0, 500));
    return;
  }
  ...
  console.log(`[MSG] type=${msg.type} id=${msg.id}`, JSON.stringify(msg).substring(0, 100));
});

ws.on('error', (e) => console.log('[ERROR]', e.message));
ws.on('close', () => { console.log('[CLOSED]'); process.exit(0); });
setTimeout(() => { console.log('[DONE]'); ws.close(); process.exit(0); }, 15000);
